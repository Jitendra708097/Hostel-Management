...
// --- ICONS ---
import { FaUser, FaEnvelope, FaLock, FaGraduationCap, FaRestroom, FaCalendarAlt, FaUniversity, FaArrowRight, FaEye, FaEyeSlash } from 'react-icons/fa';
...
const RegisterForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setNotice({ type: 'success', message: 'Registration successful. Opening your dashboard...' });
      ...
      if (error?.message?.includes('already registered') || error?.message?.includes('duplicate')) {
        ...
        setNotice({ type: 'error', message: error?.message || error?.error || 'Registration failed. Please check your details and try again.' });
      }
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-200 p-6 font-sans">
        <Motion.div
         ...
         transition={{ duration: 0.6, ease: 'easeOut' }}>
          ...
          {notice && (
            <div className={`mb-6 rounded-lg border p-3 text-sm ${notice.type === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-800'}`}>
              {notice.message}
            </div>
          )}
               ...
               { id: 'password', label: 'Password', type: 'password', icon: FaLock, placeholder: '••••••••' },
                ...
                <Motion.div className="relative" variants={itemVariant} key={field.id}>
                    ...
                    <div className="relative">
                        ...
                        {field?.id === 'password' && (
                          <button type="button" onClick={() => setShowPassword(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 p-1" aria-label={showPassword ? 'Hide password' : 'Show password'}>
          ...
          <Motion.p className="text-center text-gray-500 mt-8"
           initial={{ opacity: 0 }} 
           animate={{ opacity: 1 }} 
           transition={{ delay: 0.8 }}>Already have an account?{' '} <Link to="/login" className="text-indigo-600 hover:underline font-medium">Login here</Link>
          </Motion.p>
        </Motion.div>
    </div>
  );
