...
// MOCK WARDEN ID - In a real app, this would come from an auth context after login
//const WARDEN_USER_ID = 'YOUR_WARDEN_OBJECT_ID'; // <-- IMPORTANT: Replace with a real Warden ID from your DB
...
const attendanceDashboard = () => {
  ...
  useEffect(() => {
    const fetchSessionData = async () => {
      setIsLoading(true);
      setError('');
    //   setAuthHeader(WARDEN_USER_ID); // Set auth header for the request
      try {
        const response = await axioClient.get('/attendance/session');
  ...
  const handleSync = async () => {
    ...
    setError('');
    try {
      await axioClient.post('/attendance/mark', {
  ...
  const handleFinalize = async () => {
    if (!window.confirm('Are you sure you want to finalize? This will mark all other students as absent.')) {
      return;
    }
    setIsLoading(true);
    setError('');
    try {
      const response = await axioClient.post('/attendance/finalize');
...
//     console.log("student: ",student) || student?.userName.toLowerCase().includes(searchTerm.toLowerCase()) || student?.year.toLowerCase().includes(searchTerm.toLowerCase())
//   );
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto bg-white rounded-lg shadow-lg">
        ...
        <p className="text-lg text-red-600 font-semibold mb-6">
          Total Absent Students: {finalReport.absentCount}
        </p>
    ...
    <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="sticky top-0 bg-gray-50 py-4 z-10">
          <input
            type="text"
            placeholder="Search by name or course year..."
          ...
          <div className="flex flex-col md:flex-row gap-4 mt-4">
            <button
              onClick={handleSync}
              disabled={isSyncing}
              className="flex-1 bg-blue-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-600 disabled:bg-blue-300 transition-colors"
            >
              {isSyncing ? 'Syncing...' : `Sync Progress (${presentStudentIds.size})`}
            </button>
            <button
              onClick={handleFinalize}
              disabled={isLoading}
              className="flex-1 bg-green-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-green-600 disabled:bg-gray-400 transition-colors"
            >
              Finalize Attendance
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-6">
          {students.map(student => (
            <div
              key={student._id}
              onClick={() => handleTogglePresent(student._id)}
              className={`p-3 rounded-lg text-center cursor-pointer border-2 transition-all duration-200 ${
            ...
            >
              <img
                src={student.profileURL || 'https://via.placeholder.com/100'}
